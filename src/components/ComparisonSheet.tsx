...
const ComparisonSheet = () => {
  ...
  const downloadCSV = () => {
    const header = ["Country", "Population", "Current Demand (GWh/mo)", "Avg kWh/Household", "2031 Projected (GWh/mo)", ...displaySources.map(s => `${s} (%)`)];
  ...
  const SortHeader = ({ label, field, className = "" }: { label: string; field: SortKey; className?: string }) => (
    <th
      className={`px-3 py-3 text-left text-xs font-display cursor-pointer hover:text-neon-cyan transition-colors select-none whitespace-nowrap ${className}`}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/70 text-muted-foreground">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-display text-neon-green whitespace-nowrap">
                  <TrendingUp className="w-3 h-3 inline mr-1" />Growth
                </th>
                {displaySources.map(s => (
                  <th key={s} className="px-3 py-3 text-center text-xs font-display text-muted-foreground whitespace-nowrap">
                    {s} %
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {sorted.map((c, idx) => {
                ...
                return (
                  <tr
                    key={c.code}
                    className={`border-t border-border/50 transition-colors hover:bg-muted/30 ${idx % 2 === 0 ? "" : "bg-muted/10"}`}
                  >
                    <td className="px-3 py-2 font-display text-foreground whitespace-nowrap">{c.name}</td>
                    ...
                    <td className="px-3 py-2 font-display text-neon-cyan">
                      {c.totalDemandGwhMonth >= 1000 ? `${(c.totalDemandGwhMonth / 1000).toFixed(0)}K` : c.totalDemandGwhMonth}
                    </td>
                    <td className="px-3 py-2 font-body text-neon-yellow">{c.avgKwhPerHousehold}</td>
                    <td className="px-3 py-2 font-display text-neon-orange">
                      {future >= 1000 ? `${(future / 1000).toFixed(0)}K` : future}
                    </td>
                    <td className="px-3 py-2 font-body text-neon-green">+{growth}%</td>
                    {displaySources.map(s => (
                      <td key={s} className="px-3 py-2 text-center font-body text-muted-foreground">
                        {c.sources[s] ? (
                          <span className="inline-flex items-center">
                            <span
                              className="inline-block w-2 h-2 rounded-full mr-1"
                              style={{ backgroundColor: c.sources[s] > 30 ? "hsl(150, 100%, 45%)" : c.sources[s] > 15 ? "hsl(45, 100%, 55%)" : "hsl(220, 30%, 50%)" }}
                            />
                            {c.sources[s]}
                          </span>
                        ) : "—"}
