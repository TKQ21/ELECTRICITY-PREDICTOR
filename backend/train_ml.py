...
from models import build_models, evaluate, importance
...
def run_pipeline(
    ...
    for i, (name, family, est) in enumerate(zoo):
        ...
        try:
            ...
            results.append({"name": name, "family": family, "error": str(exc)[:200]})
            continue
    ...
    payload: dict[str, Any] = {
        ...
        "best": best,
        "baseline": baseline,
        "testDates": [t.strftime("%Y-%m-%d") for t in dates.iloc[n_train + n_val:]],
        "testActual": [float(v) for v in yte],
        "forecast": forecast,
    ...
    registered = log_comparison(payload, best_est)
